<div>
    <div class="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        {{-- Header --}}
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6 mb-8 pb-6 border-b border-[#2a3545]">
            ...
            <div class="text-center sm:text-left">
                <h1 class="font-['Exo_2'] text-2xl sm:text-3xl font-extrabold tracking-[4px] uppercase text-[#f0ece0]">★ Star Wars</h1>
                <p class="text-[10px] sm:text-xs tracking-[2px] uppercase text-[#7a8fa0] mt-1">Greatest Watch Order In The Galaxy</p>
            </div>
        ...
        {{-- Controls --}}
        <div class="flex flex-col sm:flex-row flex-wrap gap-4 items-start justify-between mb-7">
            <div class="flex flex-col gap-2.5 flex-1 w-full sm:w-auto">
                ...
                <div>
                    ...
                    @if($legendOpen)
                        <div class="mt-3 border border-[#2a3545] rounded-md overflow-hidden">
                            <div class="grid grid-cols-1 sm:grid-cols-4 divide-y sm:divide-y-0 sm:divide-x divide-[#2a3545]">
                                <div class="px-4 py-3">
                ...
                <div class="mb-2">
                    ...
                    @foreach ($group as $item)
                        ...
                        @if ($item['type'] === 'film')
                            ...
                            <div class="relative flex items-center h-20 sm:h-24 rounded-md overflow-hidden mb-1.5 transition-all duration-150
                                        hover:translate-x-1 border border-slate-800
                                        hover:border-blue-500 hover:shadow-[0_0_8px_rgba(59,130,246,0.7)]"
                                ...
                                {{-- Order number --}}
                                <div class="relative w-9 sm:w-[52px] text-center shrink-0">
                                    <span class="font-['Exo_2'] text-[10px] sm:text-xs font-bold tracking-wide {{ $entry->watched ? 'text-[#2a3f4f]' : 'text-[#7a8fa0]' }}">
                                ...
                                {{-- Info --}}
                                <button class="relative flex-1 px-2 sm:px-4 min-w-0 text-left bg-transparent border-none cursor-pointer"
                                        wire:click="toggleExpanded({{ $entry->id }})">
                                    <div class="text-[10px] sm:text-xs tracking-[1.5px] uppercase {{ $entry->watched ? 'text-[#2a3f4f]' : 'text-[#7a8fa0]' }} mb-0.5">Film</div>
                                    <div class="font-['Exo_2'] text-sm sm:text-base font-semibold tracking-wide truncate {{ $entry->watched ? 'text-[#3a4f5f]' : 'text-[#f0ece0]' }}">{{ $entry->title }}</div>
                                    <div class="text-[10px] sm:text-xs text-[#7a8fa0] mt-0.5">{{ $entry->timeline }} · {{ $entry->formatted_duration }}</div>
                                </button>

                                {{-- Badge --}}
                                <div class="relative flex items-center gap-1 sm:gap-1.5 px-2 sm:px-4 shrink-0 {{ $entry->watched ? 'opacity-30' : '' }}">
                                    <span class="w-1.5 h-1.5 rounded-full shrink-0
                                        ...
                                        : 'bg-[#7a8fa0]')) }}"></span>
                                    <span class="text-[9px] sm:text-xs tracking-[1px] sm:tracking-[1.5px] uppercase font-semibold whitespace-nowrap
                                        ...
                                        : 'text-[#9aaabb]')) }}">
                                        @if($entry->recommendation === 'must') Must
                                        @elseif($entry->recommendation === 'highly_recommended') <span class="hidden sm:inline">Highly </span>Rec.
                                        @elseif($entry->recommendation === 'recommended') Rec.
                                        @else Skip @endif
                                    </span>
                                </div>
                                ...
                                {{-- Watched toggle --}}
                                <button class="relative w-9 sm:w-[52px] h-full flex items-center justify-center bg-transparent border-none cursor-pointer shrink-0 group"
                                        wire:click="toggleWatched({{ $entry->id }})">
                                    <div class="w-[20px] h-[20px] sm:w-[22px] sm:h-[22px] rounded-full border flex items-center justify-center transition-all duration-200
                            ...
                            @if($expandedId === $entry->id)
                                <div class="bg-[#0f1520] border border-[#2a3545] border-t-0 rounded-b-md -mt-1.5 mb-1.5 overflow-hidden">
                                    @if($entry->synopsis)
                                        <div class="px-4 sm:px-6 py-4 sm:pl-[70px] border-b border-[#2a3545]">
                                    ...
                                    @if($entry->watched)
                                        <div class="px-4 sm:px-6 py-4 sm:pl-[70px]">
                                            <div class="flex items-center gap-2 mb-2">
                        ...
                        @else
                            ...
                            {{-- Group banner --}}
                            <div class="relative flex items-center h-20 sm:h-24 rounded-md {{ $isOpen ? 'rounded-b-none' : '' }} overflow-hidden mb-1.5
                                        transition-all duration-200 ease-out hover:translate-x-1 cursor-pointer
                                        border border-slate-800 shadow-[0_0_4px_rgba(59,130,246,0.15)]
                                        hover:border-blue-500 hover:shadow-[0_0_12px_rgba(59,130,246,0.85),inset_0_0_6px_rgba(59,130,246,0.25)]"
                                ...
                                {{-- Order range --}}
                                <div class="relative w-9 sm:w-[52px] text-center shrink-0">
                                    <span class="font-['Exo_2'] text-[9px] sm:text-[10px] font-bold tracking-wide text-[#7a8fa0] leading-tight">
                                ...
                                {{-- Title + meta --}}
                                <div class="relative flex-1 px-2 sm:px-4 min-w-0">
                                    <div class="text-[10px] sm:text-xs tracking-[1.5px] uppercase text-[#7a8fa0] mb-0.5">
                                        Series · {{ $epCount }} episodes
                                    </div>
                                    <div class="font-['Exo_2'] text-sm sm:text-base font-semibold tracking-wide truncate {{ $allWatched ? 'text-[#3a4f5f]' : 'text-[#f0ece0]' }}">
                                        {{ $item['series_name'] }}
                                        <span class="text-[#7a8fa0] font-normal text-xs sm:text-sm hidden sm:inline">— {{ $epCount }} episodes</span>
                                    </div>
                                    <div class="text-[10px] sm:text-xs text-[#7a8fa0] mt-0.5">
                                        {{ $first->timeline }}
                                ...
                                {{-- Chevron --}}
                                <div class="relative px-2 sm:px-4 shrink-0 text-[#7a8fa0]">
                            ...
                            @if($isOpen)
                                <div class="border border-[#2a3545] border-t-0 rounded-b-md mb-1.5 overflow-hidden">
                                    @foreach($episodes as $entry)
                                        <div class="relative flex items-center h-12 sm:h-14 transition-all duration-150 hover:translate-x-1 {{ !$loop->last ? 'border-b border-[#2a3545]' : '' }}"
                                            ...
                                            <div class="relative w-2 sm:w-6 shrink-0"></div>

                                            <div class="relative w-8 sm:w-[46px] text-center shrink-0">
                                                <span class="font-['Exo_2'] text-[9px] sm:text-[10px] font-bold tracking-wide {{ $entry->watched ? 'text-[#2a3f4f]' : 'text-[#7a8fa0]' }}">
                                                    {{ str_pad($entry->order, 2, '0', STR_PAD_LEFT) }}
                                                </span>
                                            </div>
                                            ...
                                            <button class="relative flex-1 px-2 sm:px-4 min-w-0 text-left bg-transparent border-none cursor-pointer"
                                                ...
                                                <div class="flex items-center gap-1.5 sm:gap-2">
                                                    <div class="font-['Exo_2'] text-sm sm:text-base font-semibold tracking-wide truncate {{ $entry->watched ? 'text-[#3a4f5f]' : 'text-[#f0ece0]' }}">
                                                        {{ $entry->title }}
                                                    </div>
                                                    <div class="flex items-center gap-1 shrink-0">
                                                        <span class="px-1 sm:px-1.5 py-0.5 rounded text-[10px] sm:text-[11px] font-bold tracking-wide bg-[#1a2535] text-[#4a9eca] border border-[#2a3a50]">
                                                            S{{ $entry->season }}
                                                        </span>
                                                        <span class="px-1 sm:px-1.5 py-0.5 rounded text-[10px] sm:text-[11px] font-bold tracking-wide bg-[#251a10] text-[#c9a227] border border-[#3a2a10]">
                                                            E{{ $entry->episode }}
                                                        </span>
                                                    </div>
                                                </div>
                                            ...
                                            {{-- Badge --}}
                                            <div class="relative flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 shrink-0 {{ $entry->watched ? 'opacity-30' : '' }}">
                                                <span class="w-1.5 h-1.5 rounded-full shrink-0
                                                    {{ $entry->recommendation === 'must' ? 'bg-[#c9a227]'
                                                    : ($entry->recommendation === 'highly_recommended' ? 'bg-[#9b6dff]'
                                                    : ($entry->recommendation === 'recommended' ? 'bg-[#4a9eca]'
                                                    : 'bg-[#7a8fa0]')) }}"></span>
                                                <span class="text-[9px] sm:text-[11px] tracking-[1px] uppercase font-semibold whitespace-nowrap
                                                    {{ $entry->recommendation === 'must' ? 'text-[#c9a227]'
                                                    : ($entry->recommendation === 'highly_recommended' ? 'text-[#9b6dff]'
                                                    : ($entry->recommendation === 'recommended' ? 'text-[#4a9eca]'
                                                    : 'text-[#9aaabb]')) }}">
                                            ...
                                            <button class="relative w-9 sm:w-[52px] h-full flex items-center justify-center bg-transparent border-none cursor-pointer shrink-0 group"
                                                ...
                                                <div class="w-[18px] h-[18px] sm:w-[20px] sm:h-[20px] rounded-full border flex items-center justify-center transition-all duration-200
                                                    {{ $entry->watched ? 'bg-[#c9a227] border-[#c9a227]' : 'border-[#7a8fa0] group-hover:border-[#c9a227]' }}">
                                                    @if($entry->watched)
                                        ...
                                        @if($expandedId === $entry->id)
                                            <div class="bg-[#0d1218] border-b border-[#2a3545] overflow-hidden">
                                                @if($entry->synopsis)
                                                    <div class="px-4 sm:px-5 py-3 sm:pl-[100px] border-b border-[#2a3545]">
                                                        <p class="text-sm text-[#9aaabb] leading-relaxed italic">{{ $entry->synopsis }}</p>
                                                    </div>
                                                @endif
                                                <div class="px-4 sm:px-5 py-3 sm:pl-[100px] border-b border-[#2a3545]">
                                                ...
                                                @if($entry->watched)
                                                    <div class="px-4 sm:px-5 py-3 sm:pl-[100px]">
                                                        <div class="flex items-center gap-2 mb-2">
                                                            <span class="text-[11px] tracking-[3px] uppercase text-[#c9a227]">After you watch</span>
                                                            <div class="flex-1 h-px bg-[#2a3545]"></div>
                                                            <span class="text-[10px] tracking-[2px] uppercase text-[#556070]">spoilers</span>
